...
<body>
    ...
    <h4 align="center">Laporan Daftar Pemeliharaan Aset</h4>
        ...
        <tbody>
            ...
            @foreach($records as $record)
                <tr>
                    ...
                    <td>{{ $record->location_service }}</td>
                    <td>{{ $record->AssetMaintenance->name }}</td>
                    <td>{{ $record->service_type }}</td>
                    <td>{{ $record->desc }}</td>
                    <td>Rp {{ number_format($record->service_cost, 0, ',','.') }}</td>
                </tr>
                @php
                    $totalServiceCost += $record->service_cost;
                @endphp
            @endforeach
            <!-- Baris total -->
            <tr>
                <td colspan="5"></td> <!-- Kosongkan kolom sebelum kolom total -->
                <td><strong>Total</strong></td> <!-- Merge sampai kolom Catatan -->
                <td><strong>Rp {{ number_format($totalServiceCost, 0, ',','.') }}</strong></td> <!-- Total Biaya Service -->
            </tr>
        </tbody>
    </table>
